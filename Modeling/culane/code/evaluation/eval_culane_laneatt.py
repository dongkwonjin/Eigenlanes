...
class Evaluation_CULane_LaneATT(object):
    def __init__(self, cfg=None):
        self.cfg = cfg

    def settings(self, test_mode):
        self.out_dir = self.cfg.dir['out'] + test_mode + '/'

        self.width = 30
        self.official = True
        self.sequential = False

        self.anno_dir = self.cfg.dir['dataset'] + 'img/'
        self.pred_dir = self.cfg.dir['out'] + test_mode + '/results/'
        self.list_path = self.cfg.dir['out'] + f'{test_mode}/list/datalist.txt'
    ...
    def eval_predictions(self):
        ...
        if self.sequential:
            results = t_map(partial(culane_metric, width=self.width, official=self.official, img_shape=img_shape), predictions,
                            annotations)
        else:
            results = p_map(partial(culane_metric, width=self.width, official=self.official, img_shape=img_shape), predictions,
                            annotations)
        save_pickle(dir_name=self.pred_dir, file_name='results', data=results)
