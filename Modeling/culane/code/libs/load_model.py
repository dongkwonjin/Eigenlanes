...
def load_model_for_test(cfg, dict_DB):
    if cfg.run_mode == 'test_paper':
        checkpoint = torch.load(cfg.dir['weight_paper'] + f'checkpoint_culane_res_{cfg.backbone}')
    else:
        if cfg.param_name == 'trained_last':
            checkpoint = torch.load(cfg.dir['weight'] + 'checkpoint_final')
        elif cfg.param_name == 'max_laneatt':
            checkpoint = torch.load(cfg.dir['weight'] + f'checkpoint_max_fscore_laneatt_culane_res_{cfg.backbone}')
    model = Model(cfg=cfg)
