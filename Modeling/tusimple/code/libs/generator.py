...
class Label_Generator(object):
    ...
    def compute_iou_score(self):
        ...
        self.iou_upper_score = torch.FloatTensor([]).cuda()
        for i in range(self.gt_num):
            ...
            iou_upper_score = self.measure_IoU(lane_mask[:, :h2], self.cand_mask[:, :h2])

            iou_score_tot = iou_score * w + iou_upper_score * (1 - w)
            self.iou_score = torch.cat((self.iou_score, iou_score_tot.view(1, -1)), dim=0)
    ...
    def run(self, batch, data):
        ...
        for i in range(b):
            ...
            self.gt_num = int(batch['gt_num'][i])
