...
class Preprocessing(object):
    ...
    def approximate_lane(self):
        ...
        coeff = torch.matmul(U_t, (px_coord / (self.cfg.width - 1)))
        px_coord_ap = torch.matmul(U, coeff) * ((self.cfg.width - 1))
        ...
        return {'px_coord_ap': to_np(px_coord_ap),  'c': to_np(coeff), 'exist': out['exist']}

    def construct_lane_matrix(self):
        datalist = load_pickle(self.cfg.dir['pre1'] + 'datalist')

        for i in range(len(datalist)):
            if i % self.cfg.sampling_interval != 0 and ('train' in self.cfg.datalist):  # sampling due to memory limitation
                continue
            img_name = datalist[i]
            data = load_pickle(self.cfg.dir['pre1'] + img_name)
            for j in range(0, 1):  # 1: horizontal flip
                if j == 1:
                    if self.cfg.data_flip == False:
                        continue
                    if self.cfg.datalist == 'test':
                        break
                for k in range(self.cfg.max_lane_num):
