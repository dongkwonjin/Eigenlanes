...
class Preprocessing(object):
    ...
    def clustering(self):
        ...
        if self.cfg.cluster_mode == 'kmeans':
            ...
            if self.cfg.save_pickle == True:
                save_pickle(dir_name=self.cfg.dir['out'] + 'pickle/',
                            file_name=self.cfg.cluster_mode,
                            data=cluster_result)

            cluster_result = load_pickle(self.cfg.dir['out'] + 'pickle/' + self.cfg.cluster_mode)
            candidates = cluster_result[0]  # declare centroids as lane candidates
        ftime += (time.time() - t)
        ...
        if self.cfg.display == True:
            for i in range(candidates.shape[0]):
                ...
                self.visualize.draw_polyline(data=to_np(node_pts), name='candidates_' + self.shape, ref_name='candidates_' + self.shape,
                                             color=(0, 255, 0))
        ...
        for i in range(len(datalist)):
            ...
            for j in range(0, 2):  # 1: horizontal flip
                ...
                for k in range(data_c.shape[1]):
                    ...
                    if self.shape == 'curve' and curve_check == 1:
                        self.data = torch.cat((self.data, c.view(-1, 1)), dim=1)
                        self.data_x = torch.cat((self.data_x, x_coord.view(-1, 1)), dim=1)
    ...
    def run(self):
        ...
        for shape in shape_list:
            ...
            if self.cfg.save_pickle == True:
                save_pickle(self.cfg.dir['out'] + 'pickle/', 'lane_mask_sample_{}'.format(self.shape), lane_mask)

            lane_mask = load_pickle(self.cfg.dir['out'] + 'pickle/lane_mask_sample_{}'.format(self.shape))
            for iter in range(self.cfg.n_clusters[shape] // self.cfg.n_clusters[self.shape + '_iter']):
                ...
                if self.cfg.display_all == True:
                    ...
                    file_name = 'candidates_' + self.shape + '_' + str(self.cfg.n_clusters[self.shape]) + '_' + str(iter) + '.jpg'
                ...
                if self.cfg.display_all == True:
                    dir_name = self.cfg.dir['out'] + 'display/'
                    file_name = 'candidates_' + self.shape + '_' + str(self.cfg.n_clusters[self.shape]) + '.jpg'
